...
def production_file(init_dir, data_i, noise_i, peaks_i, start_i, test='yes', condition='withnoise', window_duration=64):
    ...
    if condition == 'withnoise':
        ...
        #change directory
        os.chdir('/Volumes/Noe/DIRECTORY_withnoise_E1_'+ init_dir +'-'+str(window_duration)+test)

        #create a file for the spectrogram of the i-th data file
        file_name = 'spectrogram_E1-withnoise'+ init_dir + str(start_i) + '-2048-' + str(window_duration) +'.npy'
        ...
        file_name = 'information_E1_ft_withnoise'+ init_dir + str(start_i) + '-2048-' + str(window_duration) +'.npy'
        with open(file_name, 'wb') as file:
            #writing the file
            np.save(file, ft, allow_pickle=True, fix_imports=True)
            file.close()
            
        file_name = 'labels_withnoise_'+ init_dir + str(start_i) + '-2048-' + str(window_duration) +'.npy'
        with open(file_name, 'wb') as file:
            #writing the file
        ...
        #change directory
        os.chdir('/Volumes/Noe/DIRECTORY_withoutnoise_E1_'+init_dir +'-'+str(window_duration)+test)
        
        #create a file for the spectrogram of the i-th noise file
        
        file_name = 'spectrogram_E1-withoutnoise'+ init_dir + str(start_i) + '-2048-' + str(window_duration) +'.npy'
        with open(file_name, 'wb') as file:
            #writing the file
            np.save(file, data, allow_pickle=True, fix_imports=True)
            file.close()
        ...
        #create a file for the spectrogram of the i-th data file with the frequency and time information
        file_name = 'information_E1_ft_withoutnoise'+ init_dir + str(start_i) + '-2048-' + str(window_duration) +'.npy'
        with open(file_name, 'wb') as file:
            #writing the file
            np.save(file, ft, allow_pickle=True, fix_imports=True)
            file.close()
            
        file_name = 'labels_withoutnoise_'+ init_dir + str(start_i) + '-2048-' + str(window_duration) +'.npy'
        with open(file_name, 'wb') as file:
            #writing the file
            np.save(file, labels_i, allow_pickle=True, fix_imports=True)
            file.close()
        ...
        #saving the mean
        file_name = 'mean_withoutnoise.txt'
        if not os.path.exists(file_name):
            mean = mean_i
            with open(file_name, 'w') as f:
                f.write(str(mean))
...
def production_directory(test='yes', n_files_produce=100, init_dir = 'scale_80', condition = 'withnoise', window_duration=64):
    ...
    if condition == 'withnoise':
        #create a directory
        os.chdir('/Volumes/Noe')
        path_dir = 'DIRECTORY_withnoise_E1_'+ init_dir +'-'+str(window_duration)+test
        ...
        #create a directory
        os.chdir('/Volumes/Noe')
        path_dir = 'DIRECTORY_withoutnoise_E1_'+ init_dir +'-'+str(window_duration)+test
        if not os.path.exists(path_dir):
            os.mkdir(path_dir)
        onlyfiles = next(os.walk(path_dir))[2]
    ...
    for i in range(start_file, n_files_produce):
        ...
        if init_dir == 'scale_1':
            file_name_i = '/Volumes/Noe/'+'scale_80'+'/'+init_dir[0:5].upper() + '_80' +'_NOISY_SIGNAL_E1-'+str(start_i)+'-2048.gwf'
        else:
            file_name_i = '/Volumes/Noe/'+init_dir+'/'+init_dir[0:5].upper() + init_dir[5:] +'_NOISY_SIGNAL_E1-'+str(start_i)+'-2048.gwf'
        ...
        if init_dir == 'scale_1':
            ...
            data_i = (np.array(data_i) - np.array(noise_i))/80 + np.array(noise_i)
            data_i = TimeSeries(data_i, t0 = start_i, sample_rate = 8192)
            
        #extract corresponding peaks
        index_peaks_i = np.where('E-E1_STRAIN_NOISE-'+str(start_i)+'-2048.gwf' == OGfiles)[0]
...
#inputs = ['scale_10', 'scale_20', 'scale_50']
#inputs = ['scale_1']
#for input3 in inputs:
    #print('Dossier '+ input3)
